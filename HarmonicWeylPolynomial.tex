...
We use the incomplete Gamma function in combination with Laplace's method to evaluate the limit of the integrand.  It is necessary to split the domain into three regions, $t < \alpha$, $\alpha \leq t < 1$ and $1 < t$. We begin by treating the case $t<\alpha.$ 	We have the following identity for the incomplete Gamma function when $n\in\mathbb{N}$,
...
For $t>\alpha$ we will use the endpoint analysis of the Laplace method as outlined in ~\cite[\S3.3]{Miller}.  We begin by using the incomplete gamma function as before, defining $\kappa=\alpha+O(\frac{1}{n})$ so that $a_{\kappa n}$ plays the role of $\alpha_m$ and $\kappa n\in\mathbb{N}$
...
\begin{align*}\frac{\sqrt{r_1^2+r_2^2-2r_{12}^2}}{n^{3/2}tr_3^2}=&\frac{\sqrt{(r_1-r_2)^2+2(r_1r_2-r_{12}^2)}}{n^{3/2}tr_3^2}
\\\le&\frac{r_1-r_2}{n^{3/2}tr_3^2}+\frac{\sqrt{2(r_1r_2-r_{12}^2)}}{n^{3/2}tr_3^2}.
\end{align*}
Then we note that
...
Moreover,
\[\frac{r_1r_2-r_{12}^2}{n^3t^2r_3^4}=\frac{(a_n+a_m)c_nc_m-c_nb_m^2-c_mb_n^2}{n^3t^2(a_n+a_m)^3}.\]
...
\[\frac{\sqrt{b_n^2a_nb_{m}}}{n^{3/2}tr_3^3}=\frac{a_{n-1}\sqrt{ta_na_{m-1}}}{(a_n+a_m)^3}\le\sqrt{\frac{ta_{m-1}}{a^3_n}}\le\left\{\begin{array}{ll}
\sqrt{t},& t<\delta\\t^{-(3n-m)/2},&t>\delta
\end{array}\right.\]which is integrable for $\delta>1$ and $m\le n-4$.
\[\frac{\sqrt{b_na_na_{n-1}b_m}}{n^{3/2}tr_3^3}=\frac{a_{n-1}\sqrt{a_na_{m-1}}}{n^{1/2}(a_n+a_m)^3}\le \sqrt{\frac{a_{m-1}}{na^3_n}}\le\left\{\begin{array}{ll}
n^{-1/2},& t<\delta\\n^{-1/2}t^{-(3n-m+1)/2},&t>\delta
...
Interestingly $I_{n,0} \approx \chi_{|z|\leq 1}$  (in fact by (\ref{lim:fixed}) we have equality as $n \rightarrow \infty$).
\begin{figure}[ht]
